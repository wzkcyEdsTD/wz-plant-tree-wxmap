  ...
  <div id="myRzry">
    ...
    <ul class="list">
      <li v-for="(list,index) in dataList" :key="index">
        <!-- 图片 -->
        <div class="img">
          <img :src="`http://hcl.mynatapp.cc${list.tree_pic}`" />
        </div>
        <div class="text">
          <p>{{list.tree_name}}</p>
          <p>项目名称：马鞍池公园</p>
          <p>认养时间：{{list.maintain_startTime}}</p>
          <p>到期时间：{{list.maintain_endTime}}</p>
          <p>价格：￥{{list.sapling_price}}元</p>
          <x-button :gradients="['#28ce86', '#28ce86']" class="btn" link="/detailsRecord">养护信息</x-button>
          <x-button :gradients="['#fda421', '#fda421']" class="btn" @click.native="showGYP">公益牌</x-button>
        </div>
      </li>
    </ul>
